---
title: LOTUS, a Wikidata hosted open naturaL prOducTs occUrrences databaSe
venue: 2nd RFMF Junior Webinar Series
date: '2021-04-01'
# location: online
# image: ""
# image-alt: ""
categories:
  - Talk
author: 
- name: 
    given: Adriano 
    family: Rutz
  orcid: 0000-0003-0443-9902
  number: 1
  affiliations:
    - ref: unige
affiliations:
  - id: unige
    number: 1
    name: University of Geneva
    department: 
      - School of Pharmaceutical Sciences
      - Institute of Pharmaceutical Sciences of Western Switzerland
    group: Wolfender
    address: Rue Michel-Servet, 1
    city: Geneva
    country: Switzerland
    postal-code: 1211
description: |
  Advocating to the metabolomic community for the [LOTUS initiative](https://lotus.nprod.net/). [Available here](https://lotus.nprod.net/lotus-presentations/20210401_rfmf/index.html).
---
